...
In this project, I built regression models using two tree-based algorithms, `random forest` and `CatBoost` along with `linear regressions` with L1 and L2 regularization respectively. After comparing the performances, I constructed a final regression model using Catboost to predict used car price.   
...
knitr::include_graphics("../results/target_distribution.svg")
```
...
knitr::include_graphics("../results/price_brand_year.svg")
```
...
model_comparison <- read_csv("../results/training_scores.csv") 
...
Therefore, we selected `Catboost` as the best performing model to conduct hyperparameter optimization and tuned the maximum depth of the tree-based model (`max_depth`) to further eliminate overfitting. 

According to Figure 4, as expected, `year` was a driver for used cars prices. It was among the top three factors that affected used car prices in this model. The other two were the size of the engine (`engineSize`) and mileage per gallon (`mpg`). Some brands, especially the ones commonly perceived as premium, such as Audi, Mercedes and BMW, were among the most prominent factors that affected used car prices. They tended to push the prices higher. On the other hand, economic brands, such as Hyudai, tended to have lower prices.  

```{r shap, fig.align = 'center', echo=FALSE, fig.cap="Figure 4. Main Drivers for Used Car Prices", out.width = '50%'}
knitr::include_graphics("../results/shap.png")
```

## Results & Discussion

In the end, the tuned `Catboost` model with a maximum depth of 8 achieved an R-squared of `r percent(test_score$R_squared, 0.1)` and root mean square error of `r round(test_score[3], 2)`. The top three factors that influenced used car prices were size of the engine, year the car was built, and miles per gallon (mpg) according to the model. 

To further improve the model in the future, I would like to carefully examine the outliers in the used car prices and conduct hyperparameter optimization for `random forest` model for comparison with the tuned `CatBoost` model. Moreover, stacking and averaging ensemble techniques can also be explored. 
